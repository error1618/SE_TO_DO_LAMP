...
function user_get_logged_user(){
    ...
    if (isset($_COOKIE['app_email']) and strlen($_COOKIE['app_email'])>0){		
		$items = $db->query("SELECT * FROM users WHERE LOWER(email) = ? and pass= ? and user_id = ?",$_COOKIE['app_email'], $_COOKIE['app_pass'], $_COOKIE['id'])->fetchAll();
		if (count($items)>0){
			$appuser=$items[0];	
		}
	}
